...
const user1 = {
  owner: 'tunji',
  transactions: [0.1, 0.02, -1.00, 2.00, -0.50, -1.00, 0.19, 1.12],
  password: 'tunji1111',
};

const user2 = {
  owner: 'divine',
  transactions: [1.80, 1.20, -1.50, -1.09, 1.21, -1.04, 1.04, 0.09],
  password: 'divine2222',
};

const user3 = {
  owner: 'daniel',
  transactions: [1.10, -2.01, 1.04, -1.00, -1.27, 1.09, 1.05, 2.00],
  password: 'daniel3333',
};
...
const displayMovements = function(transactions, sort = false) {
    ...
    trans.forEach(function(tran, i) {
        ...
        <div class="movements__row">
          ...
          <div class="movements__value">${tran} ETH</div>
        </div>
        `;
...
const calcDisplayBalance = function(acc) {
    ...
    labelBalance.textContent = `${acc.balance.toFixed(2)} ETH`;
};
...
const calcDisplaySummary = function(transactions) {
    ...
    labelSumIn.textContent = `${incomes.toFixed(2)} ETH`;

    const outcomes = transactions.filter(tran => tran < 0).reduce((acc, tran) => acc + tran, 0);
    labelSumOut.textContent = `${Math.abs(outcomes.toFixed(2))} ETH`;
};
...
//GETTING THE PRICES OF ETHEREUM IN DOLLARS AND NAIRA 
//American Price
const request = new XMLHttpRequest();
request.open('GET', 'https://api.cryptonator.com/api/full/eth-usd');
request.send();
...
requestNGN.open('GET', 'https://api.cryptonator.com/api/full/eth-ngn');
requestNGN.send();
